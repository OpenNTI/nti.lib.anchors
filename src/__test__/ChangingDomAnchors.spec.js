...
describe('Tests with Changing Documents', () => {
	...
	function insertElement(daddy, bigBrother, tag, attrs) {
		...
		if (daddy) {
			daddy.insertBefore(sonny, bigBrother);
		}
		return sonny;
	}
	...
	describe('Fuzzy Anchoring Tests', () => {
		it('Single paragraph', () => {
			...
			let t1 = addTextNode(
				p,
				'This is some somewhat but not particularly long text for readers with short attention spans.'
			);

			let range = makeRange(t1, 13, t1, 47);
			let rangeDescription = createRangeDescriptionFromRange(
				range,
				document
			).description;
			...
			t1.data =
				'This is some somewhat but not particularly long text for readers with short attention spans. Here are some extra words.';
			...
			expect(
				getCurrentNodeMatches(pointer, walker)[0].confidence
			).toBeCloseTo(0.458);
		});
		...
		it('Multiple paragraphs with in-between additions', () => {
			...
			let start = rangeDescription.getStart();
			let startWalker = document.createTreeWalker(
				div,
			...
			let endWalker = document.createTreeWalker(
				div,
				NodeFilter.SHOW_TEXT
			);
			...
			expect(
				getCurrentNodeMatches(start, startWalker)[0].confidence
			).toEqual(1);
			expect(getCurrentNodeMatches(end, endWalker)[0].confidence).toEqual(
				1
			);
			...
			/*let t15 = */ addTextNode(
				span,
				'Here are some extra words in a span'
			);
		...
		it('Multiple paragraphs with empty paragraphs added outside the range', () => {
			...
			let preStartWalker = document.createTreeWalker(
				div,
				NodeFilter.SHOW_TEXT
			);
			preStartWalker.currentNode = t0;

			let afterEndWalker = document.createTreeWalker(
				div,
				NodeFilter.SHOW_TEXT
			);
			...
			expect(getCurrentNodeMatches(start, preStartWalker).length).toEqual(
				0
			);
			expect(
				getCurrentNodeMatches(end, endWalker)[0].confidence
			).toBeCloseTo(0.666);
			expect(getCurrentNodeMatches(end, afterEndWalker).length).toEqual(
				0
			);
		});
		...
		it('Multiple paragraphs with fals-matching paragraphs added outside the range', () => {
			...
			let t0 = addTextNode(
				p0,
				'This is some somewhat misleading (to the anchoring system) introductory text'
			);
			let p3 = addElement(div, 'p', {});
			let t3 = addTextNode(
				p3,
				'And more text containing things normal text containing which is quite hard to find'
			);
			...
			expect(
				getCurrentNodeMatches(start, preStartWalker)[0].confidence
			).toBeCloseTo(0.535);
			expect(getCurrentNodeMatches(end, afterEndWalker).length).toEqual(
				2
			);
			expect(
				getCurrentNodeMatches(end, afterEndWalker)[0].confidence
			).toBeCloseTo(0.679);

			// console.log(getCurrentNodeMatches(end, afterEndWalker));

			expect(
				getCurrentNodeMatches(end, afterEndWalker)[1].confidence
			).toBeCloseTo(0.476);
	...
	describe('Roundtrip Tests', () => {
		it('Can do roundtrips with modifications all within a textnode', () => {
			...
			let range = makeRange(t1, 13, t1, 47);
			let result = createRangeDescriptionFromRange(range, document)
				.description;
